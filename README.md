...
项目采用 Docker Compose 部署。首次启动项目，需要首先构建前后端镜像。在项目根目录执行：
...
前端服务启动后，通过浏览器访问前端容器暴露的地址即可使用系统。默认为8080端口，即 `http://127.0.0.1:8080/` 。

前端会通过 Nginx 将 `/api/` 前缀的请求代理到后端接口。
